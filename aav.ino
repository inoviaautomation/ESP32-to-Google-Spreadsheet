...
char pass[] = "Aaba1234";
String GOOGLE_SCRIPT_ID = "AKfycbwDby4s3sZapLTNSFK8fIYE5J_sDigAkMbeZ3LqDQLLZuHBHANtPMaChjPFCwn2e3BH";    // change Gscript ID
...
void loop() {
  ...
  if (!mb.slave()) {
    ...
    mb.readHreg(1, 132, val, 2, cb); // Slave id is 1 and register address is 40133 for Voltage line to line
    //we are reading 2 bytes from the register and saving in val
    while(mb.slave()) { // Check if transaction is active
      mb.task();
      delay(10);
   ...
   // Serial.println(val[0]);
    ...
    delay (1000);
  String urlFinal = "https://script.google.com/macros/s/"+GOOGLE_SCRIPT_ID+"/exec?"+"voltage="+ String(voltage)+ "&voltagell=" + String(voltageL)+ "&current=" + String(amp)+ "&watts=" + String(wt)+ "&frq=" + String(frq)+ "&power=" + String(wh);
